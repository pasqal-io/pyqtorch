...
def create_controlled_batch_from_operation(
    ...
    controlled_batch: torch.Tensor = (
        torch.eye(2**(n_control_qubits+1), dtype=torch.cdouble).unsqueeze(2).repeat(1, 1, batch_size)
    )
    controlled_batch[2**(n_control_qubits+1)-2:, 2**(n_control_qubits+1)-2:, :] = torch.clone(operation_batch)
