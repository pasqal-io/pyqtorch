...
from pyqtorch.core.utils import _vmap_gate
...
class HamEvo(torch.nn.Module):
    ...
    def apply(self, state: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.n_steps):
            k1 = -1j * _vmap_gate(_state, self.H, self.qubits, self.n_qubits, batch_size)
            k2 = -1j * _vmap_gate(
...
class HamEvoEig(HamEvo):
    ...
    def apply(self, state: torch.Tensor) -> torch.Tensor:
        ...
        return _vmap_gate(state, evol_operator, self.qubits, self.n_qubits, self.batch_size)
...
class HamEvoExp(HamEvo):
    ...
    def apply(self, state: torch.Tensor) -> torch.Tensor:
        ...
        return _vmap_gate(state, evol_operator, self.qubits, self.n_qubits, batch_size)
