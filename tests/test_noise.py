...
def test_damping_gates(
    ...
    if DampingGate == AmplitudeDamping:
        ...
        assert torch.allclose(DampingGate(target, error_param=(1, 1))(rho_1), rho_0)
...
def test_digital_noise_apply(
    ...
    in comparison with the noisy version with error_param = 0.
    """
    op: type[Primitive]
    error_param: float | tuple[float, ...]

    if noise_type == DigitalNoiseType.PAULI_CHANNEL:
        error_param = (0.0, 0.0, 0.0)
    elif noise_type == DigitalNoiseType.GENERALIZED_AMPLITUDE_DAMPING:
        error_param = (0.0, 0.0)
    else:
        error_param = 0.0

    noise_concrete = NoiseProtocol(noise_type, error_param)
