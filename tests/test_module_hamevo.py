...
@pytest.mark.parametrize(
    ...
    [(pyq.HamEvoType.RK4, pyq.HamEvo), (pyq.HamEvoType.EIG, pyq.HamEvoEig), (pyq.HamEvoType.EXP, pyq.HamEvoExp),
    ("rk4", pyq.HamEvo), ("eig", pyq.HamEvoEig), ("exp", pyq.HamEvoExp)]
)
def test_hamiltonianevolution_with_types(ham_evo_type: pyq.HamEvoType, ham_evo_class: torch.nn.Module) -> None:
    n_qubits = 4
    H = Hamiltonian(1)
    t_evo = torch.tensor([torch.pi / 4], dtype=torch.cdouble)
