...
def test_sample_run_expectation_grads_with_embedding_observable(embedding_fixture):
    ...
    obs_embedded = pyq.Observable([Scale(pyq.Z(0), param_name='mul_sinx_y')])  # Embedded observable
    ...
    assert torch.allclose(exp_embedded, exp_manual, atol=1e-6), \
    ...
    assert torch.allclose(exp_obs_embed, exp_obs_eval, atol=1e-6), \
    ...
    assert torch.allclose(expected_embedded_value, actual_embedded_value, atol=1e-6), \
...
def test_embedding_full_support_consistency(embedding_fixture):
    ...
    tensor_full_manual = expand_operator(tensor_local_embed, local_support, full_support)
    
    assert torch.allclose(tensor_full_embed, tensor_full_manual, atol=1e-6), \
    ...
    assert torch.allclose(tensor_full_embed, tensor_full_eval, atol=1e-6), \
    ...
    assert torch.allclose(result_embed, result_manual, atol=1e-6), \
    ...
    assert torch.allclose(result_circuit_embed, result_circuit_eval, atol=1e-6), \
    ...
    tensor_direct_full = minimal_gate.tensor(values, embedding, full_support=full_support)
    
    assert torch.allclose(tensor_expanded, tensor_direct_full, atol=1e-6), \
...
def test_embedding_diagonal_operations(embedding_fixture):
    ...
    import pyqtorch as pyq
    from pyqtorch.composite import Scale
    from pyqtorch.utils import todense_tensor, random_state
    ...
    for gate in diagonal_gates:
        ...
        assert torch.allclose(tensor_dense_embed, tensor_dense_eval, atol=1e-6), \
        ...
        assert torch.allclose(tensor_diag_embed, tensor_diag_eval, atol=1e-6), \
        ...
        assert torch.allclose(tensor_dense_embed, dense_from_diag, atol=1e-6), \
        ...
        if hasattr(gate, 'qubit_support'):
            ...
            assert torch.allclose(result_dense, result_manual, atol=1e-6), \
