...
def test_param_gate_embed(embedding_fixture):
    ...
    import random
    from pyqtorch.primitives import (
        OPS_PARAM, RX, RY, RZ, PHASE, CRX, CRY, CRZ, CPHASE, U
    )
    from pyqtorch.utils import random_state, ATOL, RTOL
    ...
    print(f"📊 Coverage: {coverage_tested}/{coverage_total} gates tested "
        f"({coverage_percentage:.1f}%)")
    # Detailed analysis
    single_qubit_gates = [gate for gate in tested_gates if gate in ["RX", "RY", "RZ", "PHASE"]]
    controlled_gates = [gate for gate in tested_gates if gate in ["CPHASE", "CRX", "CRY", "CRZ"]]
    ...
    tested_controlled = [gate for gate in controlled_rotation_gates if gate in tested_gates]
    ...
    if len(tested_controlled) == 3:
        ...
        print(f"⚡ Partial success: {len(tested_controlled)}/3 controlled rotation gates working")
        print(f"   ✅ Working: {tested_controlled}")
        failed_controlled = [gate for gate in controlled_rotation_gates if gate in skipped_gates]
