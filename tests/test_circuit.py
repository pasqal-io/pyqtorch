...
def test_embedding_with_composite_gates(embedding_fixture):
    ...
    import torch
    import pyqtorch as pyq
    from pyqtorch.composite import Sequence, Add, Scale
    ...
    assert torch.allclose(result_sequence_embed, result_sequence_eval, atol=1e-6), \
    ...
    assert torch.allclose(result_add_embed, result_add_eval, atol=1e-6), \
    ...
    assert torch.allclose(result_scale_embed, result_scale_eval, atol=1e-6), \
    ...
    assert torch.allclose(result_scale_seq_embed, result_scale_seq_eval, atol=1e-6), \
    ...
    for op, name in [(sequence_op, "Sequence"), (add_op, "Add"), (scale_op, "Scale")]:
        ...
        assert torch.allclose(tensor_embed, tensor_eval, atol=1e-6), \
